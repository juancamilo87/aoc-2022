...
data class ReverseL(val x: Int = 2, val y: Long) : Rock {
    ...
    return when (jet) {
      ...
      Right -> {
        if (x == 4) this
          ...
          occupiedSpace.contains(toRockKey(x + 3, y)) ||
          occupiedSpace.contains(toRockKey(x + 3, y + 1)) ||
          occupiedSpace.contains(toRockKey(x + 3, y + 2))
        ) this
        else copy(x = x + 1)
      }
    }
  ...
  override fun moveAndDropNoCheck(jets: List<Jet>): Rock {
    ...
    return ReverseL(newX, y - jets.size)
  }
