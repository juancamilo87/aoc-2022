...
data class VLine(val shape: IntArray, val x: Int, val y: Long, private val originalY: Long) : Rock {
  ...
  override fun move(jet: Char, occupiedSpace: ArrayDeque<Int>): Rock {
    val lines = listOf(
      ...
      Rock.getLine(occupiedSpace, originalY, y + 3)
    )
    return when (jet) {
      '<' -> {
        if (x == 0) this
  ...
  override fun moveAndDropNoCheck(jets: List<Char>): Rock {
    ...
    return VLine(newX, y - (jets.size - 1), originalY)
  }
